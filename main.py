...
class Line(object):
    def __init__(self):
        ...
        self.best_fit = None  # average of polynomial coefficients over the last n iterations
...
class Line_detector(object):
    ...
    def mask_image_by_average_lines(self, converted_img, width=30):
        ...
        for yv, ll in zip(self.yvals, self.left_line.bestx):
            image[yv, int(ll - width):int(ll + width)] = converted_img[yv, int(ll - width):int(ll + width)]
        for yv, rl in zip(self.yvals, self.right_line.bestx):
            image[yv, int(rl - width):int(rl + width)] = converted_img[yv, int(rl - width):int(rl + width)]
        return image

    def mask_image_by_lines(self, original_image, width=10):
        image = np.zeros_like(original_image)
        for yv, ll in zip(self.yvals, self.left_line.bestx):
            image[yv, int(ll - width):int(ll + width)] = 1
        for yv, rl in zip(self.yvals, self.right_line.bestx):
            image[yv, int(rl - width):int(rl + width)] = 1
        return image
    ...
    def cal_bestx_and_fit(self, weight=0.2):
        ...
        self.left_line.bestx = self.left_line.best_fit[0] * self.yvals ** 2 + self.left_line.best_fit[1] * self.yvals + \
                               self.left_line.best_fit[2]
        self.right_line.best_fit = self.right_line.best_fit * (1 - weight) + self.right_line.current_fit * weight
        self.right_line.bestx = self.right_line.best_fit[0] * self.yvals ** 2 + self.right_line.best_fit[
            1] * self.yvals + self.right_line.best_fit[2]
        self.count = 0
